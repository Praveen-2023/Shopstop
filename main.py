...
<<<<<<< HEAD
        public_routes = ['/login', '/', '/static', '/health', '/api/info', '/db-check']
=======
...
<<<<<<< HEAD
        if not hasattr(g, 'user_data'):
            log_unauthorized_access(request, "No user data found")
            return jsonify({"error": "Authentication required"}), 401

        # Print user data for debugging
        print(f"User data in admin_required: {g.user_data}")

        # Check if role is admin (case-insensitive)
        user_role = g.user_data['Role']
        if user_role.lower() != 'admin':
            log_unauthorized_access(request, f"Non-admin attempted admin action. Role: {user_role}")
...
def check_role():
    if hasattr(g, 'user_data'):
        return jsonify({
            "status": "success",
            "user_data": {
                "member_id": g.user_data['MemberID'],
                "role": g.user_data['Role'],
                "is_admin": g.user_data['Role'].lower() == 'admin'
            }
        })
    else:
        return jsonify({
...
def db_check():
    try:
        # Check cs432g17 database
        cur = mysql.connection.cursor()
        cur.execute("SELECT 'cs432g17' as db, 'connected' as status")
        g17_status = cur.fetchone()

        # Check cs432cims database
        cur.execute(f"SELECT 'cs432cims' as db, 'connected' as status FROM {CIMS_DB}.members LIMIT 1")
        cims_status = cur.fetchone()

        # Check members table in cs432cims
        cur.execute(f"SELECT COUNT(*) as count FROM {CIMS_DB}.members")
        members_count = cur.fetchone()

        # Check Login table in cs432cims
        cur.execute(f"SELECT COUNT(*) as count FROM {CIMS_DB}.Login")
        login_count = cur.fetchone()

        # Check MemberGroupMapping table in cs432cims
        cur.execute(f"SELECT COUNT(*) as count FROM {CIMS_DB}.MemberGroupMapping")
        mapping_count = cur.fetchone()
        ...
        return jsonify({
            ...
            "databases": {
                "cs432g17": g17_status,
                "cs432cims": cims_status
            },
            "table_counts": {
                "members": members_count,
                "login": login_count,
                "member_group_mapping": mapping_count
            }
        })
    except Exception as e:
        return jsonify({"status": "unhealthy", "error": str(e)}), 500
