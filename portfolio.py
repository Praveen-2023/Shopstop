...
<<<<<<< HEAD
            # Check if the requester belongs to the same group
            # Convert requester_id to int for MemberGroupMapping table
            requester_id_int = int(requester_id)
            cur.execute(f"""
                SELECT GroupID FROM {CIMS_DB}.MemberGroupMapping
                WHERE MemberID = %s AND GroupID = %s
            """, (requester_id_int, GROUP_ID))
...
<<<<<<< HEAD
    @app.route('/portfolio/member/<int:member_id>', methods=['PUT'])
=======
...
>>>>>>> 7d3483573724e77da9871ee364481b5913d41064
            ...
            cur.execute(f"""
<<<<<<< HEAD
                SELECT ID as member_id, name, UserName as username, Email as email
=======
...
<<<<<<< HEAD
            ...
            # Use direct URLs to randomuser.me instead of local files
            gender_path = 'women' if gender == 'f' else 'men'
            image_id = 10 + image_number
            member['profile_image'] = f'https://randomuser.me/api/portraits/{gender_path}/{image_id}.jpg'
=======
            try:
                gender = 'f' if member_id % 2 == 0 else 'm'
                image_number = (member_id % 6) + 1
